...
* **RingBuffer for Scrolling:** Used `RingBuffer` for `Screen` and `Scrollback` to achieve **O(1) scrolling** (adding/removing lines). ~~Linear~~ O(1) access for editing ~~is negligible for terminal heights~~.
